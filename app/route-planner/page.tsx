...
export default function RoutePlannerPage() {
  ...
  return (
    <Shell>
      <h1 className="text-3xl font-semibold text-slate-950">Offices Along My Route</h1>
      <div className="mt-5 grid gap-4 rounded-lg border border-slate-200 bg-white p-4 lg:grid-cols-5">
        ...
        <button onClick={() => mutation.mutate()} disabled={mutation.isPending} className="inline-flex items-center justify-center gap-2 rounded-md bg-slate-950 px-3 py-2 text-sm font-medium text-white disabled:opacity-60"><Route className="h-4 w-4" /> Find Offices Along My Route</button>
      </div>
      ...
      {mutation.data && mutation.data.offices.length === 0 && <p className="mt-4 rounded-lg bg-white p-4">No offices were found within this route corridor. Try increasing maximum detour.</p>}
      {mutation.data && mutation.data.offices.length > 0 && (
        <section className="mt-6 rounded-lg border border-slate-200 bg-white p-4">
          <h2 className="text-xl font-semibold">Offices Along Your Route</h2>
